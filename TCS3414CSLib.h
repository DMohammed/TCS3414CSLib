...
#ifndef _TCS3414CS_H_
#define _TCS3414CS_H_

#define TCS3414CS_ADDR 0x49 // the I2C address for the TCS3414CS

// *** COMMAND REGISTER
// Commands
#define CMD_WRITE 0x80 // Command for writing bytes into registers
#define CMD_CLEARINT                                                           \
  0xE0 // Clear any pending interrupt and is a writeonce-to-clear field
...
#define REG_LOW_THRESH_LOW_BYTE                                                \
  0x08 // ADC interrupt source lower byte of the low threshold.
#define REG_LOW_THRESH_HIGH_BYTE                                               \
  0x09 // ADC interrupt source upper byte of the low threshold.
#define REG_HIGH_THRESH 0x0A
#define REG_HIGH_THRESH_LOW_BYTE                                               \
  0x0A // ADC interrupt source lower byte of the high threshold.
#define REG_HIGH_THRESH_HIGH_BYTE                                              \
  0x0B // ADC interrupt source upper byte of the high threshold.
#define REG_BLOCK_READ                                                         \
  0x0F // write in this block to read the following registers in one block
       // message
...
#define CTL_ADC_EN                                                             \
  0x03 // This field enables the four ADC channels to begin integration.
#define CTL_ADC_VALID                                                          \
  0x10 // This read-only field indicates that the ADC channel has completed an
       // integration cycle.
#define CTL_POWER                                                              \
  0x01 // Writing a 1 powers on the device, and writing a 0 turns it off.
...
#define SYNC_EDGE                                                              \
  0x40 // If SYNC_EDGE is low, the falling edge of the sync pin is used to stop
       // an integration cycle when INTEG_MODE is 11. If SYNC_EDGE is high, the
       // rising edge of the sync pin is used to stop an integration cycle when
       // INTEG_MODE is 11.
#define INTEG_MODE_FREE                                                        \
  0x00 // the integrator is free-running and one of the three
       // internally-generated Nominal Integration Times is selected for each
       // conversion
#define INTEG_MODE_MANUAL                                                      \
  0x10 // Manually start/stop integration through serial bus using ADC_EN field
       // in Control Register.
#define INTEG_MODE_SYN_SINGLE                                                  \
  0x20 // Synchronize exactly one internally-timed integration cycle as
// specified in the NOMINAL INTEGRATION TIME beginning 2.4 μs after being
// initiated by the SYNC IN pin.
#define INTEG_MODE_SYN_MULTI                                                   \
  0x30 // Integrate over specified number of pulses on SYNC IN pin (See SYNC IN
       // PULSE COUNT table below). Minimum width of sync pulse is 50 μs. SYNC
       // IN must be low at least 3.6 μs.

// Uses single, multipurpose bitmapped field to select one of three predefined
// integration times or set the number of SYNC IN pulses to count when the
// INTEG_MODE accumulate mode (11) is selected.
// NOTE: INTEG_MODE and TIME/COUNTER fields should be written before ADC_EN is
// asserted.
...
#define INTR_STOP                                                              \
  0x40 // Stop ADC integration on interrupt. When high, ADC integration will
       // stop once an interrupt is asserted. To resume operation (1) de-assert
       // ADC_EN using CONTROL register, (2) clear interrupt using COMMAND
       // register, and (3) re-assert ADC_EN using CONTROL register. Note: Use
       // this bit to isolate a particular condition when the sensor is
       // continuously integrating.

// INTR Control Select. This field determines mode of interrupt logic according
// to the table below:
...
// Analog Gain Control. This field switches the common analog gain of the four
// ADC channels. Four gain modes are provided:
#define GAIN_1 0x00
#define GAIN_4 0x10
#define GAIN_16 0x20
#define GAIN_64 0x30

// Prescaler. This field controls a 6-bit digital prescaler and divider. The
// prescaler reduces the sensitivity of each ADC integrator as shown in the
// table below:
...
class TCS3414CS {
  //    int gain;  // TODO : thought maybe we could compute some measure in
  //    uW/cm^2 using that...
  //    byte itime;
  //    int prescaler;

public:
#define TCS3414CS_FREEMODE                                                     \
  0 // typical mode, runs free every 12MS/100MS/400MS and outputs results in
    // registers
#define TCS3414CS_LEVELINTERRUPTMODE 1

  void init(int mode = TCS3414CS_FREEMODE);
  void start();
  void stop();
